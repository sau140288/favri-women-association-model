# Function for plotting the cashflow
...
plot_cashflow <-
            ...
            ...)
  {
    ...
    assertthat::assert_that(any(substr(
      names(data), 1, nchar(cashflow_var_name)
    ) %in%
    ...
    subset_data <-
      data %>% dplyr::select(dplyr::starts_with(cashflow_var_name)) %>%
    ...
    summary_subset_data <-
      suppressMessages(
        subset_data %>% dplyr::group_by(decision_option,
                                        x_scale) %>% dplyr::summarize(
    ...
    ggplot2::ggplot(summary_subset_data, ggplot2::aes(as.numeric(x_scale))) +
      ...
      ggplot2::guides(fill = ggplot2::guide_legend(reverse = T,
                                                   order = 1)) + ggplot2::labs(
                                                     x = x_axis_name,
                                                     y = y_axis_name,
                                                     fill = legend_name,
                                                     color = ""
                                                   ) + ggplot2::facet_wrap( ~ factor(decision_option,
                                                                                     labels = facet_labels)) + ggplot2::theme_bw(base_size = base_size) +
      ggplot2::theme(
        legend.margin = ggplot2::margin(-0.75,
                                        0, 0, 0, unit = "cm"),
