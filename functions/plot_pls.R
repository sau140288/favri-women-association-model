...
plot_pls <-
  function (plsrResults,
            ...
            ...)
  {
    assertthat::assert_that(class(plsrResults) == "mvr", msg = "plsrResults is not class 'mvr', please provide a valid object. This does not appear to have been generated with the 'plsr.mcSimulation' function.")
    VIP <- function(object) {
    ...
    if ("label" %in% colnames(filtered_table))
      ...
      ordered_vars <- stats::reorder(filtered_table$Variable,
                                     filtered_table$VIP)
    ggplot2::ggplot(filtered_table,
                    ggplot2::aes(VIP, ordered_vars,
                                 fill = Coefficient > 0)) + ggplot2::geom_col() + ggplot2::geom_vline(ggplot2::aes(xintercept = cut_off_line)) +
      ggplot2::scale_fill_manual(
        breaks = c(TRUE, FALSE),
        values = c(pos_color,
                   neg_color),
        labels = legend_labels
      ) + ggplot2::scale_x_continuous(expand = ggplot2::expansion(mult = c(0,
                                                                           0.01))) + ggplot2::scale_y_discrete(expand = ggplot2::expansion(add = 0.5)) +
