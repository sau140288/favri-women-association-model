...
estimate <-
            ...
            correlation_matrix = NULL)
  {
    ...
    if (any(is.null(lower) ||
            any(is.na(lower <- as.numeric(lower)))))
      stop("\"lower\" must be supplied as numeric.")
    if (any(is.null(upper) ||
            any(is.na(upper <- as.numeric(upper)))))
      stop("\"upper\" must be supplied as numeric.")
    if (length(distribution) != length(lower) ||
        length(distribution) !=
        length(upper))
    ...
    if (!is.null(marginalOptional))
      for (i in names(marginalOptional)) {
        if (i == "variable" && any(!is.character(marginalOptional[[i]])))
          stop("Optional argument \"",
               i,
               "\" is not character for all entries.")
        if (i == "method" &&
            any(!is.character(marginalOptional[[i]])))
          stop("Optional argument \"",
               i,
               "\" is not character for all entries.")
        if (i == "median") {
          ...
          marginalOptional <-
            marginalOptional[!names(marginalOptional) %in%
                               "median"]
          if (!is.null(median)) {
            ...
            median[!is.na(median) & is.character(median) &
                     median == ""] <-
              rep(NA, length(distribution))[!is.na(median) &
                                              is.character(median) &
                                              median == ""]
            if (any(is.na(median) &
                    !is.na(median <- as.numeric(median))))
              stop(
                "\"median\": all values must be one of the following: \"numeric\", \"character\" \n                 with value \"mean\", \"NA\" or \"\"."
              )
