            ...
            ...)
  {
    ...
    for (i in 1:length(EVPIresults)) {
      ...
      if (is.null(full_evpi_data))
        ...
        full_evpi_data <- dplyr::bind_rows(full_evpi_data,
                                           data)
    }
    ...
    data$output_variable <-
      factor(data$output_variable, levels = decision_vars,
    ...
    ggplot2::ggplot(data, ggplot2::aes(
      x = EVPI,
      y = stats::reorder(!!ggplot2::ensym(y_axis),
                         EVPI)
    )) + ggplot2::geom_col(fill = bar_color) + ggplot2::scale_x_continuous(
      expand = ggplot2::expansion(mult = c(0,
                                           0.01)),
      labels = scales::dollar_format(prefix = unit)
    ) +
      ggplot2::labs(y = y_axis_name, x = x_axis_name) + ggplot2::facet_wrap( ~
                                                                               output_variable,
