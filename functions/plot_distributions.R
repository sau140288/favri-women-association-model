...
plot_distributions <-
            ...
            ...)
  {
    ...
    data <-
      dplyr::rename_at(data, dplyr::vars(tidyselect::all_of(old_names)),
                       ~ new_names)
    standard_plot_data <-
      tidyr::pivot_longer(data, tidyselect::all_of(new_names))
    if (is.null(colors)) {
      colors <- c(
        "#009999",
        "#0000FF",
    ...
    standard_plot <-
      ggplot2::ggplot(standard_plot_data,
                      ggplot2::aes(x = value,
    ...
    if (method == "smooth_simple_overlay") {
      return(
        standard_plot + ggplot2::geom_density(color = NA,
                                              alpha = 0.5) + ggplot2::theme(...)
      )
    }
    if (method == "hist_simple_overlay") {
      return(
        standard_plot + ggplot2::geom_histogram(
          color = NA,
          alpha = 0.5,
          bins = 150
        ) + ggplot2::theme(...)
    ...
    if (method == "boxplot_density") {
      ...
      boxploth_width_correction <-
        stats::coefficients(regression)[[1]] +
        (stats::coefficients(regression)[[2]] * options_difference)
      return(
        ggplot2::ggplot(standard_plot_data, ggplot2::aes(x = value,
                                                         fill = name)) + ggplot2::geom_density(alpha = 0.5,
                                                                                               color = NA) + ggstance::geom_boxploth(
                                                                                                 ggplot2::aes(x = value,
                                                                                                              y = 0),
                                                                                                 width = max(
                                                                                                   stats::density(standard_plot_data$value)$y *
                                                                                                     boxploth_width_correction
                                                                                                 ),
                                                                                                 varwidth = TRUE,
                                                                                                 alpha = 0.5,
